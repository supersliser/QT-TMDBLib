...
TEST(Qtmdb_JSON_Movie_Test, alternativeTitles)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_TRUE(response.value("titles").isArray());
}
...
TEST(Qtmdb_JSON_Movie_Test, keywords)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("keywords").toArray()[0].toObject().value("name").toString(), "dual identity");
}
...
TEST(Qtmdb_JSON_Movie_Test, recommendations)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("results").toArray()[0].toObject().value("title").toString(), "Pulp Fiction");
}
...
TEST(Qtmdb_JSON_Movie_Test, releaseDates)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("results").toArray()[0].toObject().value("release_dates").toArray()[0].toObject().value("note").toString(), "Blu-ray");
}
TEST(Qtmdb_JSON_Movie_Test, reviews)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("results").toArray()[0].toObject().value("author").toString(), "Goddard");
}
...
TEST(Qtmdb_JSON_Movie_Test, watchProviders)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("results").toObject().value("GB").toObject().value("flatrate").toArray()[0].toObject().value("provider_name").toString(), "Disney Plus");
}