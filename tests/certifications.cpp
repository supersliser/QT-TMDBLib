...
TEST(Qtmdb_JSON_Certifications_Test, movie)
{
    ...
    EXPECT_EQ(response.value("GB").toArray()[0].toObject().value("certification").toString(), "15");
}
...
TEST(Qtmdb_JSON_Certifications_Test, tv)
{
    ...
    EXPECT_FALSE(response.isEmpty());
    EXPECT_EQ(response.value("GB").toArray()[0].toObject().value("certification").toString(), "U");
}